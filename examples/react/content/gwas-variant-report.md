...
Every component on this page is mounted by `MarkdownRenderer` from `@basemark/react` — the same generic `@lit/react` wrapper used on every other page in this example, not per-component code. This page's own markdown lives in `content/gwas-variant-report.md`, loaded via Vite's `?raw` import — not a JS string.
