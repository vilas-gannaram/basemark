...
- **Never declare a custom element class at module scope.** `class X extends HTMLElement` touches `HTMLElement` the moment the file loads — breaks any DOM-less import (Node tests, the CLI under Node). Declare the class _inside_ its `register*` function, behind `typeof HTMLElement !== 'undefined'`. See `core/error-element.ts` for the pattern; every `common`/`bio` component follows it.
...
- tsup's programmatic `build()` API auto-discovers and merges the calling package's own `tsup.config.ts` even when called from a script with its own inline options — pass `config: false` to opt out. See `packages/cli/scripts/bundle-runtime.ts`, which builds separate runtime bundles from a script in a package that also has its own main `tsup.config.ts`.
