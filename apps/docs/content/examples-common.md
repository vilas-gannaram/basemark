You're the newest hire in the lab that ran every variant and locus on the [Bio examples](/examples/bio) page — this is the onboarding page new sequencing-core hires actually get pointed at on day one. Rendered server-side via `renderMarkdownToHtml()` into static HTML, then upgraded client-side into real custom elements. Nothing on this page is bio-specific, though — it's `@basemark/common`, and this exact markdown also renders unchanged through `examples/vanilla`'s `renderMarkdown()` and `examples/react`'s `MarkdownRenderer`.
